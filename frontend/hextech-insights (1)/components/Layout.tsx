...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const headerSearchHiddenRoutes = new Set(['/profile', '/compare', '/models', '/predictions']);
  const hideHeaderSearch = headerSearchHiddenRoutes.has(location.pathname);

  const navItems = [
    { label: 'Dashboard', path: '/dashboard', icon: 'dashboard' },
    { label: 'Live Analytics', path: '/profile', icon: 'query_stats' },
    { label: 'Compare Players', path: '/compare', icon: 'balance' },
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background-dark">
      ...
      <motion.aside
        ...
        className="w-64 flex-shrink-0 bg-[#0a1428] border-r border-primary/20 flex flex-col hidden lg:flex"
      >
        ...
        <header className="h-16 border-b border-primary/20 flex items-center justify-between px-8 bg-background-dark/80 backdrop-blur-md z-10">
          ...
          <div className="flex items-center gap-6 ml-8">
            <div className="flex gap-4">
              <Link to="/dashboard" className="text-sm font-bold text-primary hover:text-white transition-colors">Global</Link>
            </div>
